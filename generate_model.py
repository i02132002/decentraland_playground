...
baseline_data['price_usd_sm'] = pd.Series.rolling(baseline_data.price_usd, window=WINDOW).mean()
baseline_data.price_usd_sm.interpolate(method='backfill', inplace = True)
baseline_data['price_sm'] = pd.Series.rolling(baseline_data.price, window=5).mean()
baseline_data.price_sm.interpolate(method='backfill', inplace = True)

cleaned_transactions = transactions[['category','price','updatedAt','updatedAt_dt', 'x', 'y','parcel_id']].copy()
...
cleaned_transactions['updatedAt_dt'] = cleaned_transactions['updatedAt_dt'].apply(lambda x: datetime.strptime(x,'%Y-%m-%d'))
cleaned_transactions['price_usd'] = cleaned_transactions[['updatedAt_dt','price']].apply(MANA_to_USD, price_table = (mana_price), axis='columns')
cleaned_transactions['total_price_usd'] = cleaned_transactions[['updatedAt_dt','total_price']].apply(MANA_to_USD, price_table = (mana_price), axis='columns')
cleaned_transactions = cleaned_transactions.join(baseline_data.price_sm, on=cleaned_transactions.updatedAt_dt)
cleaned_transactions.rename(columns={'price_sm':'avg_parcel_price'}, inplace = True)
cleaned_transactions['norm_price'] = cleaned_transactions.price/cleaned_transactions.avg_parcel_price
...
class FeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        feats = X[['id', 'category', 'updatedAt', 'updatedAt_dt', 'x', 'y', 'estate_size']].copy()
        sold_parcels_xy = [i for i in zip(X.x, X.y)]
        feats['dist_road'] = self.calculate_dist(ROAD_PARCELS, sold_parcels_xy)
        feats['dist_main_road'] = self.calculate_dist(MAIN_ROAD_PARCELS, sold_parcels_xy)
        feats['dist_genesis'] = self.calculate_dist(GENESIS_PARCELS, sold_parcels_xy)
        feats['dist_district_x'] = self.calculate_dist(list(zip(district_x.x, district_x.y)), sold_parcels_xy)
        feats['dist_DU'] = self.calculate_dist(list(zip(DU.x, DU.y)), sold_parcels_xy)
        feats['dist_DC'] = self.calculate_dist(list(zip(DC.x, DC.y)), sold_parcels_xy)
        feats['dist_casino'] = self.calculate_dist(list(zip(casino.x, casino.y)), sold_parcels_xy)

        feats['dist_center'] = np.sqrt(np.sum(np.array(sold_parcels_xy)**2,1))
        feats['const'] =1.0
        if self.isestate:
            dist_list = X.groupby(by='id').apply(lambda x: list(zip(x.x,x.y)))
            min_dist_road = dist_list.apply(lambda x: min(calculate_dist(ROAD_PARCELS,x))[0]).rename('min_dist_road',inplace = True)
            min_dist_genesis = dist_list.apply(lambda x: min(calculate_dist(GENESIS_PARCELS,x))[0]).rename('min_dist_genesis',inplace = True)
...
map_model = Pipeline([
    ('transformer', fg),
    ('selector', selector),
    ('regressor', RandomForestRegressor(n_estimators=200, max_depth=6, min_samples_split=7)),
])
...
X_train, X_test, y_train, y_test = (cleaned_transactions[:round(len_*0.8)], 
                                   ...
                                   cleaned_transactions.norm_price[:round(len_*0.8)], 
...
y_pred = model.predict(all_parcels)
all_parcels['y_pred'] = model.predict(all_parcels)
...
all_parcels[['x','y','y_pred']].to_csv('parcel_model.csv',index=False)
